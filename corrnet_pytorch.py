...
a = np.array([[1, 2, 3], [1, 1, 1], [2, 1, 2]], dtype = float)
b = np.array([[0, 1, 2], [0, 0, 0], [1, 0, 1]], dtype = float)
c = torch.from_numpy(a)
d = torch.from_numpy(b)

s1 = torch.sub(c, torch.from_numpy(np.array([1, 1, 1])))
s2 = torch.sub(d, 0)
l = torch.mul(s1, s2)
torch.sqrt(l)
torch.sum(l, dim = 1)

e = torch.from_numpy(np.array([[1, 1, 1], [1, 1, 1]], dtype = float))
f = torch.from_numpy(np.array([[1, 3, 2], [2, 2, 2]], dtype = float))
